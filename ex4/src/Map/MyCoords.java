...
public class MyCoords {
	static final int RADIOS_EARTH=6371000;
	...
	public static double find_point(double diff_radian,double p) {
	...
	private static double diff_radian(double p0, double p1) {
		double diff=p1-p0;
	...
	public static Point3D addmetertopoint (double lat, double lon) {
		double meter=(1 / ((2 *Math.PI / 360) * RADIOS_EARTH)) / 1000;
		lat=lat+meter;
		 meter=(1 / ((2 *Math.PI / 360) * RADIOS_EARTH)) / 1000;
		lon =lon+ meter / Math.cos(lat * (Math.PI/ 180));
